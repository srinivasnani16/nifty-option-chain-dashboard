...
DASHBOARD_PWD = os.getenv("DASHBOARD_PWD", "")

def check_password(entered):
    if not DASHBOARD_PWD:
        return False
    return entered == DASHBOARD_PWD
...
body { background:#0d1117; color:#e6edf3; font-family:'Segoe UI',Arial,sans-serif; font-size:14px; min-height:100vh; }
.login-wrap { min-height:100vh; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg,#0a2342,#0d1117); }
.login-box { background:#161b22; border:1px solid #30363d; border-radius:16px; padding:36px 32px; width:320px; text-align:center; box-shadow:0 8px 32px rgba(0,0,0,0.4); }
...
.login-box input { width:100%; padding:12px 16px; background:#21262d; border:1px solid #30363d; border-radius:10px; color:#e6edf3; font-size:14px; margin-bottom:14px; outline:none; transition:border 0.2s; }
.login-box input:focus { border-color:#2196f3; }
.login-btn { width:100%; padding:12px; background:linear-gradient(135deg,#1565c0,#2196f3); color:#fff; border:none; border-radius:10px; font-size:15px; font-weight:bold; cursor:pointer; transition:opacity 0.2s; }
.login-btn:hover { opacity:0.9; }
.error-msg { color:#ef5350; font-size:12px; margin-top:12px; padding:8px; background:#2d1b1b; border-radius:8px; }
.header { background:linear-gradient(135deg,#0a2342,#1a3a5c); padding:10px 14px; display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; border-bottom:2px solid #2196f3; position:sticky; top:0; z-index:200; gap:8px; }
...
.live-badge { background:#0d2137; border:1px solid #2196f3; color:#64b5f6; padding:4px 10px; border-radius:12px; font-size:11px; display:flex; align-items:center; gap:5px; }
.live-dot { width:7px; height:7px; border-radius:50%; background:#4caf50; animation:blink 1.2s infinite; }
@keyframes blink { 0%,100%{opacity:1} 50%{opacity:0.2} }
.logout-btn { color:#ef5350; font-size:11px; text-decoration:none; padding:4px 8px; border:1px solid #ef5350; border-radius:8px; }
.filter-section { background:#161b22; border-bottom:1px solid #30363d; }
.filter-label { padding:6px 12px 2px 12px; font-size:10px; text-transform:uppercase; color:#58a6ff; font-weight:bold; letter-spacing:0.5px; }
.ts-bar { padding:6px 12px 10px 12px; display:flex; gap:6px; overflow-x:auto; scrollbar-width:thin; scrollbar-color:#30363d #161b22; }
.ts-btn { background:#21262d; color:#8b949e; border:1px solid #30363d; padding:7px 14px; border-radius:10px; font-size:11px; cursor:pointer; white-space:nowrap; transition:all 0.2s; flex-shrink:0; }
...
.active-ts { background:#0d1117; padding:6px 14px; border-bottom:1px solid #30363d; display:flex; align-items:center; gap:10px; flex-wrap:wrap; }
.active-ts-badge { background:#0d2137; color:#2196f3; padding:4px 14px; border-radius:12px; font-size:12px; font-weight:bold; border:1px solid #1565c0; }
.latest-tag { background:#1b5e20; color:#a5d6a7; padding:2px 8px; border-radius:8px; font-size:10px; font-weight:bold; }
...
th { padding:7px 6px; text-align:center; font-size:11px; font-weight:700; letter-spacing:0.5px; border-bottom:2px solid #30363d; }
td { padding:7px 6px; text-align:center; border-bottom:1px solid #1c2128; font-size:12px; transition:background 0.15s; }
tr:nth-child(even) td { background:#0d1117; }
tr:nth-child(odd) td { background:#161b22; }
tr:hover td { background:#1c2128; }
.atm-row td { background:#0d2137; border-top:2px solid #2196f3; border-bottom:2px solid #2196f3; font-weight:bold; }
...
.highest-oi { background:#1b5e20; color:#a5d6a7; font-weight:bold; border-radius:5px; padding:2px 4px; }
.lowest-oi { background:#b71c1c; color:#ef9a9a; font-weight:bold; border-radius:5px; padding:2px 4px; }
.highest-vol { background:#0d47a1; color:#90caf9; font-weight:bold; border-radius:5px; padding:2px 4px; }
.drastic-coi { background:#e65100; color:#ffcc80; font-weight:bold; border-radius:5px; padding:2px 4px; }
...
.no-data { text-align:center; padding:60px 20px; color:#8b949e; font-size:14px; line-height:2; }
...
@media (min-width:1024px) {
  ...
  .chart-side { display:flex; position:sticky; top:54px; height:calc(100vh - 54px); background:#161b22; border-left:1px solid #30363d; flex-direction:column; overflow:hidden; }
  .chart-header { background:#0a2342; padding:8px 14px; border-bottom:1px solid #30363d; display:flex; align-items:center; justify-content:space-between; flex-shrink:0; }
  .chart-title { color:#2196f3; font-size:13px; font-weight:bold; display:flex; align-items:center; gap:6px; }
...
<div class="active-ts">
  ...
  {% if global_latest_ts and selected_ts == global_latest_ts %}<span class="latest-tag">🟢 LIVE MARKET SNAPSHOT</span>{% endif %}
</div>
<div class="main-content">
  <div class="table-side">
    ...
    {% else %}
    <div class="no-data"><div class="icon">📭</div>No data available for this date/time selection.<br>Data collection starts at <b>9:15 AM</b> on trading days.</div>
    {% endif %}
    </div>
    <div class="legend">
...
if(el){ setInterval(function(){ secs--; if(secs<=0){window.location.reload();} if(el) el.textContent=secs; },1000); }
...
def index():
    ...
    if not dates:
        return render_template_string(HTML, logged_in=True, dates=[], timestamps=[], selected_date="", selected_ts="", rows=[], global_latest_ts="")
    ...
    if not raw_rows:
        return render_template_string(HTML, logged_in=True, dates=dates, timestamps=timestamps, selected_date=selected_date, selected_ts=selected_ts, global_latest_ts=global_latest_ts, rows=[])
    all_oi = []
    all_vol = []
    for r in raw_rows:
    ...
    for r in raw_rows:
        ...
        rows.append({"tag":tag,"ce_oi":ce_oi,"ce_coi":ce_coi,"ce_volume":ce_vol,"ce_ltp":ce_ltp,"strike":strike,"pe_ltp":pe_ltp,"pe_volume":pe_vol,"pe_coi":pe_coi,"pe_oi":pe_oi,"ce_oi_class":oi_class(ce_oi,"ce"),"pe_oi_class":oi_class(pe_oi,"pe"),"ce_vol_class":vol_class(ce_vol,"ce"),"pe_vol_class":vol_class(pe_vol,"pe"),"ce_coi_class":coi_class(ce_coi,ce_oi),"pe_coi_class":coi_class(pe_coi,pe_oi)})
    return render_template_string(HTML, logged_in=True, dates=dates, timestamps=timestamps, selected_date=selected_date, selected_ts=selected_ts, global_latest_ts=global_latest_ts, rows=rows)
